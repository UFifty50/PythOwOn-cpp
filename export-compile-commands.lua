...
--	args = table.join(args, toolset.getundefines(cfg.undefines))
	local allDefines   = table.join(prj.defines   or {}, cfg.defines   or {})
    local allUndefines = table.join(prj.undefines or {}, cfg.undefines or {})
    args = table.join(args, toolset.getdefines(allDefines))
    args = table.join(args, toolset.getundefines(allUndefines))
...
newaction {
    ...
    execute = function()
        local dir = {}
        dir.location = _MAIN_SCRIPT_DIR .. "/.vscode"
